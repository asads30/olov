      ...
      (error, results) => {
         if (error) {
            ...
            if (results.length === 0) {
               ...
               bot.sendMessage(
                  msg.chat.id,
                  `Спасибо за уделенное время. Мы только запустили наше приложение, и Ваше мнение очень ценно для нас. 11 ноября мы объявим победителей нашего конкурса и вручим ценные призы. Оставайтесь с нами, впереди вас ждет много интересного.
Ваши ответы:
...
3) Будете ли Вы рекомендовать приложение своим друзьям/знакомым - ${var4}`
               ...
               bot.sendMessage(
                  msg.chat.id,
                  `Vaqtingiz uchun katta rahmat. Biz hozirda o'z dasturimizni ishga tushirdik va sizning fikr-mulohazalaringiz biz uchun juda qadrlidir. 11 yanvar kuni biz tanlovimiz g'oliblarini e'lon qilamiz va qimmatbaho sovg'alarni topshiramiz. Bizni kuzatib turing, sizni oldinda ko'plab qiziqarli narsalar kutmoqda.
               ...
               if (results.length === 0) {
                  ...
                  bot.answerCallbackQuery(callbackQuery.id).then(() =>
                     setTimeout(
                        bot.sendMessage(
                           msg.chat.id,
                           `Спасибо за уделенное время. Мы только запустили наше приложение, и Ваше мнение очень ценно для нас. 11 января мы объявим победителей нашего конкурса и вручим ценные призы. Оставайтесь с нами, впереди вас ждет много интересного.
Ваши ответы:
1) Откуда Вы узнали про приложение - ${var1}
2) Насколько удобно было пройти регистрацию в приложении - ${var2}
3) Будете ли Вы использовать приложение - ${var3}
